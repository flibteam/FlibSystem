...
- **О программе** — ссылки на GitHub автора, поддержку проекта и **тёмная/светлая тема**.

### Обновления и безопасность

- **Автоматическая проверка обновлений** — при запуске программа проверяет GitHub и предлагает скачать новую версию.
- **Ручная проверка** — кнопка «Проверить обновления» на странице «О программе».
- **SHA-256 проверка** — каждый релиз сопровождается файлом `.sha256`. При скачивании программа вычисляет хэш файла и сверяет с эталоном. Если хэш не совпадает — файл удаляется и показывается предупреждение.
- **Защита от модификации** — если установлена сборка новее, чем в официальном релизе, программа предупредит о возможных модификациях и порекомендует скачать официальную версию.

### Тема оформления

- **Светлая тема** (macOS Aqua) — по умолчанию.
- **Тёмная тема** (macOS Dark) — переключается в «О программе». Выбор запоминается между запусками.
